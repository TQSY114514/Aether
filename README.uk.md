...
> [!CAUTION]
> **Попередження Windows SmartScreen — це очікувано.** Aether розробляється студентом-розробником без комерційного сертифіката підпису коду, тому Windows 11 / Defender під час першого запуску може показати «Windows захистив ваш ПК».
> **Застосунок безпечний і має відкритий код — перегляньте код, а потім натисніть «Докладніше → Виконати все одно».**
> Якщо антивірус помістив його в карантин, додайте папку застосунку до виключень антивіруса (детальніше див. [Завантаження](#завантаження)). Жодні дані не залишають ваш комп'ютер, крім як до налаштованих вами LLM-провайдерів.

**Платформа: лише Windows.** Офіційні збірки, тестування та підтримка орієнтовані на Windows. macOS / Linux можна зібрати з вихідного коду, але вони офіційно не підтримуються, а підписування коду не планується — очікуйте попередження SmartScreen "невідомий видавець" під час першого запуску (див. [Завантаження](#завантаження)).
...
## Дві форми продукту, єдине ядро

Aether використовує **дворушійну архітектуру**, надаючи два рівноправні першокласні інтерфейси зі спільним агентським рантаймом, локальним сховищем SQLite та 3-рівневою пісочницею безпеки:

- 🖥️ **Aether Desktop (GUI)** — графічний інтерфейс на Electron + React з продуманою типографікою, drag-and-drop, візуальною ареною моделей та зручним центром налаштувань. **Рекомендовано для більшості повсякденних завдань і нових користувачів.** (Завантажте з [GitHub Releases](#завантаження--настільна-версія), готовий до роботи)
- ⌨️ **Aether Terminal (CLI / TUI / SDK)** — легкий термінальний UI на Ink v5 з миттєвим стартом, 100% керуванням з клавіатури, порядковим diff-узгодженням та нативною підтримкою SSH і CI/CD автоматизації. **Створено для поціновувачів термінала та скриптів.** (`npm i -g aetherai`, [встановлення →](#завантаження--cli--tui--sdk))

> 💡 **Безшовна спадкоємність**: Обидва поділяють `agentCore`, 42 інструменти, пам'ять SQLite, багатомодельну маршрутизацію, MCP-сервери та те саме сховище сесій. Чат, розпочатий у GUI, можна продовжити в TUI командою `aether tui --session <id>` — і навпаки.

---

**Де знаходиться Aether — чесна оцінка.** Самооцінка в порівнянні з 20 провідними термінальними, IDE та автономними агентами на основі відкритих даних (актуальність: 2026-09; оцінки, не бенчмарки). Ми відкрито демонструємо асиметричну форму: ведемо в локальній приватності, 3-рівневій пісочниці та мультипровайдерах; відверто визнаємо відставання в чистому коді від Claude Code і Cursor. Докладний порівняльний аналіз див. у [docs/competitive-analysis.md](docs/competitive-analysis.md).

<p align="center"><img src="./assets/agent-radar-2026.uk.svg" width="760" alt="Чесний радар самооцінки Aether vs 20 систем" /></p>

<sub>Діаграма згенерована через <a href="./app/scripts/gen-radar.cjs">app/scripts/gen-radar.cjs</a> — бали 20 інструментів вбудовані дослівно; відтворюється за допомогою <code>node app/scripts/gen-radar.cjs</code>.</sub>
...
| **Термінальний TUI** | Інтерактивний термінал на Ink v5: потік сеансів, картки інструментів, перегляд/відкат diff, клавіатурні ворота дозволів, дерево сеансів `/fork`, `/memory`, зворотне впорскування steering під час роботи. | `Experimental` |
| **Headless CLI · RPC · SDK** | CLI з чотирма режимами (одноразовий / NDJSON / JSONL RPC / конвеєр), SDK без Electron (`aetherai/sdk`), доступний для машин JSONL-протокол. | `Experimental` |
...
> Оберіть **одне**. Обидва продукти використовують той самий агентний рантайм і сховище сесій.
> - **Просто хочете настільний чат-застосунок?** → [Aether Desktop](#завантаження--настільна-версія)
> - **Хочете термінального агента / CI / SDK?** → [Aether CLI](#завантаження--cli--tui--sdk)

### Завантаження — Настільна версія
...
### Завантаження — CLI / TUI / SDK

**`aetherai`** — це npm-пакет. Він об'єднує headless CLI, інтерактивний TUI на Ink v5 та SDK без Electron в одному бінарнику.
...
`aether` і `aetherai` вказують на один і той самий пакет. Зафіксуйте версію командою `npm install -g aetherai@0.9.0`, щоб вона збігалася з десктоп-релізом.
...
Якщо ви віддаєте перевагу запуску з вихідного коду або хочете змінити код, скористайтеся `start.bat` (потрібен [Node.js 22+](https://nodejs.org)):
...
> **Два продукти або одне дерево вихідного коду** — обидва продукти живуть в одному репозиторії. `app/electron/` — спільний агентний рантайм, `app/src/` — рендерер десктопа, `app/cli.js` + `app/tui/` — точки входу CLI/TUI. Релізи позначаються git-тегами (`v*`), і з одного тегу ви отримуєте і настільний інсталятор, і публікацію в npm.

---

## Швидкий старт

**Передумови:** Node.js 22+, npm 9+
...
Створено з ❤️ за допомогою Electron + Node.js + React + TypeScript
