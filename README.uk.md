...
> **Статус: Beta.** AetherAI — це сольний/хобі-проєкт. Він працює, але очікуйте шорстких
...
## Що робить AetherAI особливим

AetherAI поєднує кілька можливостей, які зазвичай розподілені між багатьма інструментами, в один локальний настільний застосунок:
...
| **15 мов інтерфейсу** | Включно з класичною китайською (文言) та арабською RTL. | `Beta` |
...
> **Усі дані залишаються локальними.** AetherAI нічого не збирає та нічого про вас не завантажує. Ваші API-ключі, розмови та персони зберігаються в локальній базі даних SQLite. Єдині вихідні мережеві запити йдуть до LLM-провайдерів, яких ви налаштували.

---

## Розширення VS Code і Headless CLI

Окрім настільного застосунку, AetherAI постачає того самого агента як CLI та розширення редактора:
...
Окрім настільного застосунку та звичайного CLI, AetherAI постачає інтерактивний термінальний інтерфейс, доступний для машин JSONL RPC-режим і SDK без Electron. Усі три поділяють те саме ядро агента, пам'ять, персони, MCP-інструменти та правила дозволів, що й настільна версія.
...
| [MCP](https://modelcontextprotocol.io) | Специфікація, якою говорить агент AetherAI |
