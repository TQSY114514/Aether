...
> [!CAUTION]
> **Die Windows-SmartScreen-Warnung ist zu erwarten.** Aether wird von einem Studenten ohne kommerzielles Codesignatur-Zertifikat entwickelt, daher zeigen Windows 11 / Defender beim ersten Start möglicherweise „Windows hat Ihren PC geschützt" an.
> **Die App ist sicher und Open Source — prüfen Sie den Code und klicken Sie dann auf „Weitere Informationen → Trotzdem ausführen".**
> Sollte Ihre Antivirus-Software sie in Quarantäne verschieben, fügen Sie den App-Ordner zu den AV-Ausnahmen hinzu (Details siehe [Download](#download)). Es verlassen keine Daten Ihren Rechner — außer zu den von Ihnen konfigurierten LLM-Anbietern.
...
## Zwei Formfaktoren, ein einheitlicher Kern

Aether setzt auf eine **Dual-Engine-Architektur**, die zwei gleichberechtigte erstklassige Oberflächen bietet und sich dieselbe Agent-Runtime, SQLite-Speicher und 3-Stufen-Sicherheitssandbox teilt:

- 🖥️ **Aether Desktop (GUI)** — Electron + React Grafikoberfläche mit reichhaltiger Typografie, Drag & Drop, visueller Modell-Arena und intuitiven Einstellungen. **Empfohlen für die tägliche Entwicklung und alle neuen Benutzer.** (Von [GitHub Releases](#download--desktop) herunterladen, funktioniert sofort)
- ⌨️ **Aether Terminal (CLI / TUI / SDK)** — Leichtgewichtige Ink-v5-Terminal-UI mit Sofortstart, reiner Tastaturbedienung, zeilengenauer Diff-Freigabe und nativer Unterstützung für SSH & CI/CD-Pipelines. **Entwickelt für Terminal-Enthusiasten und Automatisierung.** (`npm i -g aetherai`, [Setup →](#download--cli--tui--sdk))

> 💡 **Nahtlose Kontinuität**: Beide teilen sich `agentCore`, 42 Tools, SQLite-Speicher, Multi-Modell-Routing, MCP-Server und denselben Sitzungsspeicher. Ein in der GUI begonnener Chat lässt sich in der TUI mit `aether tui --session <id>` fortsetzen — und umgekehrt.

---

**Aethers aktuelle Position — ehrlich bewertet.** Selbstbewertung im Vergleich zu 16 führenden Terminal-, IDE- und Plattform-Agenten auf Basis öffentlich zugänglicher Daten (2026-09 neueste Bewertung; Schätzungen, keine Benchmarks). Wir zeigen die asymmetrische Form ungeschönt: Führend bei lokaler Privatsphäre, 3-stufiger Sandbox-Sicherheit und Multi-Provider-Freiheit; wir räumen den Rückstand beim reinen Coding gegenüber Claude Code/Cursor offen ein. Detaillierter Vergleich siehe [docs/competitive-analysis.md](docs/competitive-analysis.md).

<p align="center"><img src="./assets/agent-radar-2026.de.svg" width="760" alt="Aether Ehrliches Selbsteinschätzungs-Radar vs. 16 Peer-Agenten" /></p>

<sub>Diagramm generiert durch <a href="./app/scripts/gen-radar.cjs">app/scripts/gen-radar.cjs</a> — Werte für alle 16 Tools eingebettet; reproduzierbar mit <code>node app/scripts/gen-radar.cjs</code>.</sub>
...
> Wählen Sie **eins**. Beide Produkte teilen sich dieselbe Agent-Runtime und denselben Sitzungsspeicher.
> - **Nur eine Desktop-Chat-App gewünscht?** → [Aether Desktop](#download--desktop)
> - **Terminal-Agent / CI / SDK gewünscht?** → [Aether CLI](#download--cli--tui--sdk)
...
**`aetherai`** ist das npm-Paket. Es bündelt die Headless-CLI, die interaktive Ink-v5-TUI und das Electron-freie SDK in einem einzigen Binary.
...
Wenn Sie lieber aus dem Quellcode ausführen oder den Code ändern möchten, verwenden Sie `start.bat` (erfordert [Node.js 22+](https://nodejs.org)):
...
> **Zwei Produkte, ein Quellcode-Baum** — beide Produkte leben im selben Repo. `app/electron/` enthält die gemeinsame Agent-Runtime, `app/src/` ist das Desktop-Renderer, `app/cli.js` + `app/tui/` sind die CLI-/TUI-Einstiegspunkte. Releases werden per Git-Tag (`v*`) markiert, und aus einem einzigen Tag entstehen sowohl ein Desktop-Installer als auch eine npm-Veröffentlichung.

---

## Schnellstart

**Voraussetzungen:** Node.js 22+, npm 9+
...
- `Experimental` **Hierarchische Planung** — generiert automatisch Aufgabenaufschlüsselungen für komplexe Anfragen.
...
| [Hermes Agent](https://github.com/NousResearch/hermes-agent) | Iterationsbudget, strukturierter Langzeitspeicher, autonome Skills, Cron-Scheduler, FTS5-Speichersuche |
| [Evolver](https://github.com/EvoMap/evolver) | Selbstevolutions-Engine, GEP (Genome Evolution Protocol) |
...
| [OpenCode](https://github.com/sst/opencode) | TUI-Tastatur-/Theme-/Berechtigungs-UX, Prompt-Cache-Policy-Layer |
| [OpenAI Codex](https://github.com/openai/codex) | Sandbox-Prozessbaum-Isolation, Laufzeit-/Statusindikator-UX |

### UI & UX

| Projekt | Inspiration |
|---|---|
| [shadcn/ui](https://github.com/shadcn-ui/ui) | cn() Copy-Paste-Komponentenmethodik |
...
| [new-api](https://github.com/QuantumNous/new-api) | Param-Shapes für reasoning-effort (Relay-Konvertierungslogik) |
...
Mit ❤️ gebaut mit Electron + Node.js + React + TypeScript
