...
**Where Aether stands.** Self-scored against 20 leading terminal, IDE, and platform agent tools from public information, using the released v0.8.2 (2026-09; subjective estimates, not benchmarks). Two axes are on par with the peer best: multi-provider freedom and local privacy. Everywhere else — raw coding, ecosystem, multi-agent orchestration, safety (app-layer sandbox, no OS-level isolation, Windows-only), and desktop/TUI polish — Aether still trails the top tier, and the chart says so. Per-axis rationale and the wider 29-tool survey live in [docs/competitive-analysis.md](docs/competitive-analysis.md).
