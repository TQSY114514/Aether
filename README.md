...
**Where Aether stands.** Self-scored against 20 leading terminal, IDE, and platform agent tools from public information, using the released v0.9.0 (2026-09; subjective estimates, not benchmarks). Two axes are on par with the peer best: multi-provider freedom and local privacy. Everywhere else — raw coding, ecosystem, multi-agent orchestration, safety (app-layer sandbox, no OS-level isolation, Windows-only), and desktop/TUI polish — Aether still trails the top tier, and the chart says so. Per-axis rationale and the wider 29-tool survey live in [docs/competitive-analysis.md](docs/competitive-analysis.md).

<p align="center"><img src="./assets/agent-radar-2026.en.svg" width="760" alt="Aether self-assessment radar vs 20 peer agents (Claude Code, Codex, Cursor, Devin Desktop, Trae, Devin, OpenHands, DSH, Amp, Gemini Code Assist, etc.)" /></p>

<sub>Chart generated by <a href="./app/scripts/gen-radar.cjs">app/scripts/gen-radar.cjs</a> — all 20 peer scores are embedded verbatim; regenerate with <code>node app/scripts/gen-radar.cjs</code>.</sub>
