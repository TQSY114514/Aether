...
- **诚实雷达（Hermes）**：自评 CSV 逐字内嵌 `app/scripts/gen-radar.cjs`，生成 `assets/agent-radar-2026.svg` 入 README 双语。禁顶格美化——2026-09 按 v0.9.0 实际交付重打分后，仅多模型与本地隐私两轴与同类最佳同档，编程 -2.8 / 生态 -5.8 / 多 Agent -3.5 / 安全 -2.8 / UX -3.3 的差距如实画出；逐轴依据见 `docs/competitive-analysis.md` 第 4 节。
- **GPT P0 四项对账现状**：
