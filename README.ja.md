...
### どんなモデルともチャット、安全なコーディングエージェント、モデルを横並びで比較 — すべてあなたのマシンで
...
複数の LLM プロバイダ — OpenAI / Claude / DeepSeek / ローカルモデル / 任意の OpenAI 互換エンドポイント — を一つのデスクトップアプリに統合します。チャット、コーディングエージェントの実行、ELO 投票付きマルチモデルアリーナでのモデル比較ができます。

**ローカルファースト設計。** API キーと会話はローカル SQLite に保存され、あなたが設定したプロバイダ以外の外部に送信されることはありません。

**デフォルトで安全。** 内蔵エージェントはワークスペースサンドボックス内で実行され、ファイルやコマンドへのアクセスは実行前に確認され、すべてのツール呼び出しは監査可能です。
