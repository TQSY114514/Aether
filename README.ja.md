...
**Aether の現在地。** 公開情報に基づき、リリース済みの v0.9.0 を主要なターミナル / IDE / プラットフォーム系エージェント20種と比較した自己評価です（2026-09；ベンチマークではなく主観的推定）。同種ベストと同水準なのは「複数プロバイダー切替」と「ローカルプライバシー」の2軸のみ。コーディング、エコシステム、マルチエージェント、セキュリティ（アプリ層サンドボックスのみ、OS レベル隔離なし、Windows 専用）、デスクトップ / TUI 体験はトップ層に届いておらず、チャートはそれをそのまま示しています。軸ごとの根拠と29種の広域調査は [docs/competitive-analysis.md](docs/competitive-analysis.md) を参照してください。

<p align="center"><img src="./assets/agent-radar-2026.ja.svg" width="760" alt="Aether 自己評価レーダー: 主要エージェント20種との比較" /></p>
