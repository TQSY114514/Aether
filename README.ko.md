...
### 어떤 모델이든 채팅하고, 안전한 코딩 에이전트를 실행하고, 모델을 나란히 비교하세요 — 모두 당신의 머신에서
...
여러 LLM 프로바이더 — OpenAI / Claude / DeepSeek / 로컬 모델 / 모든 OpenAI 호환 엔드포인트 — 를 하나의 데스크톱 앱으로 통합합니다. 채팅하고, 코딩 에이전트를 실행하고, ELO 투표가 포함된 다중 모델 아레나에서 모델을 나란히 비교하세요.

**설계상 로컬 우선.** API 키와 대화 내용은 로컬 SQLite 데이터베이스에 저장되며, 사용자가 설정한 프로바이더로 전송되는 것 외에는 절대 컴퓨터 밖으로 나가지 않습니다.

**기본적으로 안전.** 내장 에이전트는 워크스페이스 샌드박스 안에서 권한 사다리와 함께 실행됩니다: 파일 및 명령 접근은 실행 전에 확인되고, 모든 도구 호출은 감사할 수 있습니다.
