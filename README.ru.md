...
## Две формы продукта, единое ядро

Aether использует **двухдвижковую архитектуру**, предоставляя два равноправных интерфейса высшего класса с общим агентским рантаймом, локальным хранилищем SQLite и 3-уровневой песочницей безопасности:

- 🖥️ **Aether Desktop (GUI)** — графический интерфейс на Electron + React с богатой типографикой, drag-and-drop, визуальной ареной моделей и удобным центром настроек. **Рекомендуется для большинства повседневных задач и новых пользователей.** (Скачайте из [GitHub Releases](#загрузка--десктоп), готов к работе)
- ⌨️ **Aether Terminal (CLI / TUI / SDK)** — легковесный терминальный UI на Ink v5 с мгновенным стартом, 100% управлением с клавиатуры, построчным diff-согласованием и встроенной поддержкой SSH и CI/CD автоматизации. **Создан для ценителей терминала и скриптов.** (`npm i -g aetherai`, [установка →](#загрузка--cli--tui--sdk))

> 💡 **Бесшовная преемственность**: Оба используют общий `agentCore`, 42 инструмента, память SQLite, мультимодельную маршрутизацию, MCP-серверы и одно хранилище сессий. Чат, начатый в GUI, можно продолжить в TUI командой `aether tui --session <id>` — и наоборот.

---

**Где находится Aether.** Самооценка выпущенной v0.9.0 в сравнении с 20 ведущими терминальными, IDE и платформенными агентами на основе открытых данных (2026-09; субъективные оценки, не бенчмарки). Лишь две оси на уровне лучших в классе: свобода выбора провайдера и локальная приватность. В кодинге, экосистеме, оркестрации агентов, безопасности (песочница на уровне приложения, без изоляции ОС, только Windows) и зрелости desktop / TUI Aether пока отстаёт от первой группы — и диаграмма это показывает. Обоснование по осям и расширенный обзор 29 инструментов: [docs/competitive-analysis.md](docs/competitive-analysis.md).

<p align="center"><img src="./assets/agent-radar-2026.ru.svg" width="760" alt="Радар самооценки Aether vs 20 систем" /></p>

<sub>Диаграмма сгенерирована через <a href="./app/scripts/gen-radar.cjs">app/scripts/gen-radar.cjs</a> — баллы 20 инструментов встроены дословно; воспроизводится с помощью <code>node app/scripts/gen-radar.cjs</code>.</sub>
...
`aether` и `aetherai` указывают на один и тот же пакет. Закрепите версию командой `npm install -g aetherai@0.8.0`, чтобы она совпадала с десктоп-релизом.
