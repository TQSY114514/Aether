...
interface Window {
  electronAPI: {
    ...
    chat: {
      ...
      onSubagentEvent: (callback: (payload: { messageId: number; sessionId: number; event: { type: 'start' | 'done' | 'error'; id: string; index: number; task: string; status: 'running' | 'done' | 'error'; startedAt?: number; latencyMs?: number; output?: string; error?: string; childSessionId?: number } }) => void) => () => void
