...
const {
  normalizeContent: memNormalize,
  keywords: memKeywords,
  jaccard: memJaccard,
  SIMILAR_JACCARD: MEM_SIMILAR_JACCARD,
} = require("./memoryText");
...
function encryptKey(plain) {
  ...
  if (!safeStorage.isEncryptionAvailable()) {
    if (!_warnedNoEncryption) {
      ...
      log.warn(
        "[database] safeStorage 不可用，API Key 将明文存储（系统级加密不可用）",
      );
    }
    return plain;
  }
  ...
  for (const r of rows) {
    ...
    try {
      const enc = safeStorage
        .encryptString(String(r.api_key))
        .toString("base64");
      db.prepare("UPDATE provider SET api_key = ? WHERE id = ?").run(enc, r.id);
      migrated++;
    } catch {
      /* skip un-migratable row */
    }
  }
  if (migrated)
    log.info(
      `[database] 迁移了 ${migrated} 个明文 API Key 到 safeStorage 加密`,
    );
  return migrated;
}
...
function createEmptyDatabase(dbPath) {
  ...
  target.exec(
    "CREATE TABLE IF NOT EXISTS provider (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, api_url TEXT NOT NULL, api_key TEXT, api_format TEXT NOT NULL DEFAULT 'openai', enabled INTEGER NOT NULL DEFAULT 1, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)",
  );
  target.exec(
    "CREATE TABLE IF NOT EXISTS model (id INTEGER PRIMARY KEY AUTOINCREMENT, provider_id INTEGER NOT NULL, model_name TEXT NOT NULL, display_name TEXT, is_primary INTEGER NOT NULL DEFAULT 0, fallback_order INTEGER, context_window INTEGER, input_price_per_1k REAL, output_price_per_1k REAL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)",
  );
  target.exec(
    "CREATE TABLE IF NOT EXISTS persona (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, prompt TEXT NOT NULL, avatar TEXT, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)",
  );
  target.exec(
  ...
  target.exec(
    "CREATE TABLE IF NOT EXISTS model_score (id INTEGER PRIMARY KEY AUTOINCREMENT, model_id INTEGER NOT NULL, intent TEXT NOT NULL, score REAL NOT NULL DEFAULT 1000, win_count INTEGER NOT NULL DEFAULT 0, total_count INTEGER NOT NULL DEFAULT 0, UNIQUE(model_id, intent))",
  );
  target.exec(
    "CREATE TABLE IF NOT EXISTS arena_vote (id INTEGER PRIMARY KEY AUTOINCREMENT, prompt TEXT NOT NULL, intent TEXT, winner_model_id INTEGER, winner_model_name TEXT, loser_model_ids TEXT NOT NULL, loser_model_names TEXT NOT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)",
  );
  ...
  target.exec(
    "CREATE TABLE IF NOT EXISTS mcp_server (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, command TEXT NOT NULL, args TEXT, env TEXT, enabled INTEGER NOT NULL DEFAULT 1, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)",
  );
  target.exec(
    "CREATE TABLE IF NOT EXISTS memory (id INTEGER PRIMARY KEY AUTOINCREMENT, content TEXT NOT NULL, type TEXT DEFAULT 'fact', created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)",
  );
  target.exec(
    "CREATE TABLE IF NOT EXISTS repo_index_cache (workspace TEXT PRIMARY KEY, mtime_x REAL NOT NULL, graph_json TEXT NOT NULL)",
  );
  target.exec(
    "CREATE TABLE IF NOT EXISTS tool_loop_run (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id INTEGER, started_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, duration_ms INTEGER, iterations INTEGER, input_tokens INTEGER, output_tokens INTEGER, error_kind TEXT)",
  );
  target.exec(
    "CREATE TABLE IF NOT EXISTS tool_call_sample (id INTEGER PRIMARY KEY AUTOINCREMENT, run_id INTEGER, tool_name TEXT, duration_ms INTEGER, success INTEGER)",
  );

  target.exec(
    'CREATE TABLE IF NOT EXISTS kg_nodes (id INTEGER PRIMARY KEY AUTOINCREMENT, entity TEXT NOT NULL UNIQUE, type TEXT DEFAULT "entity", created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)',
  );
  target.exec(
    'CREATE TABLE IF NOT EXISTS kg_edges (id INTEGER PRIMARY KEY AUTOINCREMENT, "from" TEXT NOT NULL, "to" TEXT NOT NULL, relation TEXT NOT NULL, confidence REAL NOT NULL DEFAULT 0.8, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)',
  );
  ...
  target.exec(
    'CREATE TABLE IF NOT EXISTS provider_credential (id INTEGER PRIMARY KEY AUTOINCREMENT, provider_id INTEGER NOT NULL, api_key TEXT NOT NULL, label TEXT, enabled INTEGER NOT NULL DEFAULT 1, last_used_at DATETIME DEFAULT "2000-01-01T00:00:00.000Z", cooldown_until DATETIME, error_count INTEGER NOT NULL DEFAULT 0, disable_reason TEXT)',
  );
  ...
  try {
    target.exec("CREATE INDEX IF NOT EXISTS idx_message_session_created ON message(session_id, created_at)");
    target.exec("CREATE INDEX IF NOT EXISTS idx_session_updated ON session(updated_at DESC)");
  } catch {}
...
function initDatabase() {
  ...
  try {
    db.exec("ALTER TABLE memory ADD COLUMN relation_entity TEXT");
  } catch {}
  try {
    db.exec("ALTER TABLE memory ADD COLUMN relation_type TEXT");
  } catch {}
  try {
    db.exec("ALTER TABLE memory ADD COLUMN relation_target TEXT");
  } catch {}
  try {
    db.exec(
  ...
  try {
    db.exec("ALTER TABLE memory ADD COLUMN source_session_id INTEGER");
  } catch {}
  try {
    db.exec("ALTER TABLE memory ADD COLUMN source_turn_id INTEGER");
  } catch {}
  try {
    db.exec("ALTER TABLE memory ADD COLUMN confidence REAL DEFAULT 1.0");
  } catch {}
  try {
    db.exec("ALTER TABLE memory ADD COLUMN conflicts_with INTEGER");
  } catch {}
  ...
  try {
    db.prepare(
      "UPDATE memory SET conflicts_with = NULL WHERE conflicts_with = id",
    ).run();
  } catch {}
  ...
  // 值未变的行不写，个人应用规模开销可忽略。
  try {
    db.exec("ALTER TABLE memory ADD COLUMN content_norm TEXT");
  } catch {}
  try {
    const rows = db
      .prepare("SELECT id, content, content_norm FROM memory")
      .all();
    ...
    db.exec(
      "CREATE INDEX IF NOT EXISTS idx_memory_content_norm ON memory(content_norm)",
    );
  } catch {}
  ...
  try {
    db.exec(
      "CREATE INDEX IF NOT EXISTS idx_memory_type_time ON memory (LOWER(TRIM(type)), created_at DESC, id DESC)",
    );
  } catch {}
  ...
  // Model 去重：历史上 fetch-models 只增不删、且无唯一约束，可能累计出同一
  // provider_id+model_name 的重复行。启动时按 (provider_id, model_name) 保留
  // id 最小的一行，删除其余，再建唯一索引防止复发。
  try {
    const dups = db.prepare(`
      SELECT MIN(id) AS keep_id, provider_id, model_name, COUNT(*) AS c
      FROM model GROUP BY provider_id, model_name HAVING c > 1
    `).all()
    for (const d of dups) {
      db.prepare('DELETE FROM model WHERE provider_id = ? AND model_name = ? AND id <> ?')
        .run(d.provider_id, d.model_name, d.keep_id)
    }
    db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_model_provider_name ON model(provider_id, model_name)')
  } catch {}
  try {
    db.exec("ALTER TABLE memory ADD COLUMN origin TEXT DEFAULT 'user'");
  } catch {}
  ...
  try {
    db.exec('CREATE INDEX IF NOT EXISTS idx_kg_edges_from ON kg_edges("from")');
  } catch {}
  try {
    db.exec('CREATE INDEX IF NOT EXISTS idx_kg_edges_to ON kg_edges("to")');
  } catch {}
  try {
    db.exec("CREATE INDEX IF NOT EXISTS idx_message_session_created ON message(session_id, created_at)");
    db.exec("CREATE INDEX IF NOT EXISTS idx_session_updated ON session(updated_at DESC)");
  } catch {}

  try {
    require("./llm/checkpointManager").createTable(db);
  } catch {}
  initSkillSuccessTable();

  try {
    db.exec("ALTER TABLE evolution_events ADD COLUMN blast_radius TEXT");
  } catch (e) {}

  try {
    db.exec(
      "ALTER TABLE skill_usage ADD COLUMN state TEXT NOT NULL DEFAULT 'active'",
    );
  } catch {}
  try {
    db.exec(
      "ALTER TABLE skill_usage ADD COLUMN pinned INTEGER NOT NULL DEFAULT 0",
    );
  } catch {}
  try {
    db.exec(
      "ALTER TABLE skill_usage ADD COLUMN created_by TEXT NOT NULL DEFAULT 'user'",
    );
  } catch {}
  try {
    db.exec(
      "ALTER TABLE skill_usage ADD COLUMN patch_count INTEGER NOT NULL DEFAULT 0",
    );
  } catch {}
  try {
    db.exec("ALTER TABLE skill_usage ADD COLUMN last_viewed_at DATETIME");
  } catch {}
  try {
    db.exec("ALTER TABLE skill_usage ADD COLUMN archived_at DATETIME");
  } catch {}
  ...
  try {
    const agentTaskSql = db
      .prepare(
        "SELECT sql FROM sqlite_master WHERE type='table' AND name='agent_task'",
      )
      .get();
    if (
      agentTaskSql &&
  ...
  if (!existingKeys.includes("fallback_timeout_ms"))
    db.prepare(
      "INSERT INTO settings (key, value) VALUES ('fallback_timeout_ms', '30000')",
    ).run();
  if (!existingKeys.includes("theme"))
    db.prepare(
  ...
  if (!existingKeys.includes("lint_command"))
    db.prepare(
      "INSERT INTO settings (key, value) VALUES ('lint_command', '')",
    ).run();
  if (!existingKeys.includes("test_command"))
    db.prepare(
      "INSERT INTO settings (key, value) VALUES ('test_command', '')",
    ).run();
...
function getProviders() {
  return db
    .prepare("SELECT * FROM provider ORDER BY id")
    .all()
    .map((r) => ({ ...r, api_key: maskKey(decryptKey(r.api_key)) }));
}
...
function getProviderDecrypted(id) {
...
function addProvider({
  ...
  const info = db
    .prepare(
      "INSERT INTO provider (name, api_url, api_key, api_format, enabled) VALUES (?, ?, ?, ?, ?)",
    )
    .run(name, api_url, encrypted, api_format, enabled);
  return { lastInsertRowid: Number(info.lastInsertRowid) };
}
function updateProvider(id, data) {
  // "留空/掩码 = 不修改"（H2）：编辑表单回显掩码值或留空时，不得覆盖已存 key。
  const patch = { ...data };
  if ("api_key" in patch && isMaskedOrEmptyKey(patch.api_key))
  ...
  const values = keys.map((k) =>
    k === "api_key" ? encryptKey(patch[k]) : patch[k],
  );
  db.prepare(
    `UPDATE provider SET ${keys.map((k) => `${k} = ?`).join(", ")} WHERE id = ?`,
  ).run(...values, id);
}
...
function getModels(providerId) {
  return db
    .prepare(
      "SELECT * FROM model WHERE provider_id = ? ORDER BY fallback_order ASC, id ASC",
    )
    .all(providerId);
}
...
function addModel({
  ...
  // The unique index on (provider_id, model_name) rejects duplicate inserts;
  // fall back to returning the existing row's id so manual model adds and
  // callers that don't dedupe stay idempotent.
  try {
    const info = db
      .prepare(
        "INSERT INTO model (provider_id, model_name, display_name, is_primary, fallback_order, context_window, input_price_per_1k, output_price_per_1k) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      )
      .run(
        provider_id,
        model_name,
    ...
    if (e && String(e.message).includes("UNIQUE")) {
      const existing = db.prepare("SELECT id FROM model WHERE provider_id = ? AND model_name = ?").get(provider_id, model_name);
      if (existing) return { lastInsertRowid: Number(existing.id) };
    }
    throw e;
  }
}
function updateModel(id, data) {
  ...
  if (!keys.length) return;
  db.prepare(
    `UPDATE model SET ${keys.map((k) => `${k} = ?`).join(", ")} WHERE id = ?`,
...
function syncModels(providerId, fetchedNames) {
  const names = Array.from(
    new Set(
      (fetchedNames || []).map((n) => String(n || "").trim()).filter(Boolean),
    ),
  );
  ...
  for (const name of names) {
    if (!existingByName.has(name)) {
      db.prepare(
        "INSERT INTO model (provider_id, model_name, display_name, is_primary, fallback_order, context_window, input_price_per_1k, output_price_per_1k) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      ).run(providerId, name, null, 0, null, null, null, null);
      added.push(name);
    }
  }
  ...
  if (removed.length > 0) {
    const stillPrimary = db
      .prepare("SELECT id FROM model WHERE provider_id = ? AND is_primary = 1")
      .get(providerId);
    if (!stillPrimary) {
      const first = db
        .prepare(
          "SELECT id FROM model WHERE provider_id = ? ORDER BY id ASC LIMIT 1",
        )
        .get(providerId);
      if (first)
        db.prepare("UPDATE model SET is_primary = 1 WHERE id = ?").run(
          first.id,
        );
    }
  }
...
function getFallbackChain(providerId) {
  return db
    .prepare(
      "SELECT * FROM model WHERE provider_id = ? AND fallback_order IS NOT NULL ORDER BY fallback_order ASC",
    )
    .all(providerId);
}
...
function addPersona({ name, prompt, avatar = null }) {
  const info = db
    .prepare("INSERT INTO persona (name, prompt, avatar) VALUES (?, ?, ?)")
    .run(name, prompt, avatar);
  return { lastInsertRowid: Number(info.lastInsertRowid) };
}
function updatePersona(id, data) {
  ...
  if (!keys.length) return;
  db.prepare(
    `UPDATE persona SET ${keys.map((k) => `${k} = ?`).join(", ")} WHERE id = ?`,
  ).run(...keys.map((k) => data[k]), id);
}
...
function renameSession(id, title) {
  db.prepare("UPDATE session SET title = ?, updated_at = ? WHERE id = ?").run(
    title,
    localNow(),
    id,
  );
...
function pinSession(id, pinned = 1) {
  db.prepare("UPDATE session SET pinned = ?, updated_at = ? WHERE id = ?").run(
    pinned,
    localNow(),
    id,
  );
...
function deleteSession(id) {
  const del = db.transaction((sid) => {
    try {
      db.prepare("DELETE FROM messages_fts WHERE session_id = ?").run(sid);
    } catch {}
    ...
    try {
      db.prepare(
        "UPDATE memory SET source_session_id = NULL WHERE source_session_id = ?",
      ).run(sid);
    } catch {}
...
function touchSession(id) {
  db.prepare("UPDATE session SET updated_at = ? WHERE id = ?").run(
    localNow(),
    id,
  );
}
...
function getSessionConfig(id) {
  const row = db
    .prepare("SELECT config, persona_id FROM session WHERE id = ?")
    .get(id);
  if (!row) return null;
  try {
    return JSON.parse(row.config || "null");
  } catch {
    return null;
  }
}
function setSessionConfig(id, config) {
  db.prepare("UPDATE session SET config = ? WHERE id = ?").run(
...
function addMessage({
  ...
  const info = db
    .prepare(
      "INSERT INTO message (session_id, role, content, model_used, provider_used, token_count, latency_ms, status, error_message, arena_model) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    )
    .run(
      session_id,
      role,
  ...
  try {
    db.prepare(
      "UPDATE session SET is_placeholder = 0 WHERE id = ? AND is_placeholder = 1",
    ).run(session_id);
  } catch {}
  try {
    db.prepare(
      "INSERT INTO messages_fts (content, session_id, message_id) VALUES (?, ?, ?)",
    ).run(String(content || ""), session_id, Number(info.lastInsertRowid));
  } catch {}
  return { lastInsertRowid: Number(info.lastInsertRowid) };
}
function updateMessage(id, data) {
  ...
  if (!keys.length) return;
  db.prepare(
    `UPDATE message SET ${keys.map((k) => `${k} = ?`).join(", ")} WHERE id = ?`,
  ).run(...keys.map((k) => data[k]), id);
}
...
function updateAgentTask(id, patch) {
  const cols = Object.keys(patch || {}).filter((k) =>
    AGENT_TASK_PATCH_COLS.has(k),
  );
  if (!cols.length) return;
  const sets = cols.map((k) => `${k} = ?`).join(", ");
  const vals = cols.map((k) => patch[k]);
  db.prepare(
    `UPDATE agent_task SET ${sets}, updated_at = datetime('now') WHERE id = ?`,
  ).run(...vals, id);
}
function listAgentTasks(limit = 100) {
  return db
    .prepare("SELECT * FROM agent_task ORDER BY priority DESC, id DESC LIMIT ?")
    .all(limit)
    .map((r) => ({
      ...r,
      id: Number(r.id),
      session_id: r.session_id ? Number(r.session_id) : null,
      model_id: r.model_id ? Number(r.model_id) : null,
      priority: Number(r.priority),
      attempts: Number(r.attempts),
      max_retry: Number(r.max_retry),
    }));
}
...
function addScheduledTask({
  ...
  const info = db
    .prepare(
      "INSERT INTO scheduled_task (name, type, interval_ms, enabled, config) VALUES (?, ?, ?, ?, ?)",
    )
    .run(
      name,
      type,
...
function markScheduledTaskRun(id) {
  db.prepare("UPDATE scheduled_task SET last_run_at = ? WHERE id = ?").run(
    localNow(),
    id,
  );
}
...
function getModelScores() {
  return db
    .prepare(
      `SELECT ms.*, m.model_name, p.name as provider_name FROM model_score ms JOIN model m ON ms.model_id=m.id JOIN provider p ON m.provider_id=p.id ORDER BY ms.intent, ms.score DESC`,
    )
    .all();
}
function initModelScores(modelId) {
  for (const intent of [
    ...
    db.prepare(
      "INSERT OR IGNORE INTO model_score (model_id, intent, score, win_count, total_count) VALUES (?,?,1000,0,0)",
    ).run(modelId);
  }
}
function updateElo(winnerModelId, loserModelIds, intent) {
  ...
  const readScore = (modelId) => {
    const row = db
      .prepare("SELECT score FROM model_score WHERE model_id=? AND intent=?")
      .get(modelId, intent);
  ...
  const upsertScore = (modelId, newScore, incrementWin) => {
    ...
    db.prepare(
      "UPDATE model_score SET score = ?, win_count = win_count + ?, total_count = total_count + 1 WHERE model_id = ? AND intent = ?",
    ).run(newScore, incrementWin ? 1 : 0, modelId, intent);
  };
...
async function recordArenaVote({
  prompt,
  winnerModelId,
  winnerModelName,
  loserModelIds,
  ...
  db.prepare(
    "INSERT INTO arena_vote (prompt, intent, winner_model_id, winner_model_name, loser_model_ids, loser_model_names) VALUES (?, ?, ?, ?, ?, ?)",
  ).run(
    prompt,
    intent,
    winnerModelId,
    winnerModelName,
    JSON.stringify(loserModelIds),
    JSON.stringify(loserModelNames),
  );
...
function listArenaBenchmarks() {
  return db
    .prepare("SELECT * FROM arena_benchmark ORDER BY created_at DESC")
    .all()
    .map((r) => ({
      ...r,
...
function saveArenaBenchmark({ id = null, name, tasks, modelIds }) {
  ...
  if (id != null) {
    db.prepare(
      "UPDATE arena_benchmark SET name = ?, tasks = ?, model_ids = ? WHERE id = ?",
    ).run(name, t, m, id);
    return { id };
  }
  const info = db
    .prepare(
      "INSERT INTO arena_benchmark (name, tasks, model_ids) VALUES (?, ?, ?)",
    )
    .run(name, t, m);
...
function updateArenaBenchmarkResults(id, results, lastRun) {
  db.prepare(
    "UPDATE arena_benchmark SET results = ?, last_run = ? WHERE id = ?",
  ).run(JSON.stringify(results), lastRun, id);
}
...
function getModelLatency() {
  const rows = db
    .prepare(
      `SELECT m.id AS model_id, AVG(u.latency_ms) AS avg_latency
...
function getModelUsageMetrics() {
  return db
    ...
    .map((r) => ({
      ...
      avg_latency_ms:
        r.avg_latency_ms == null ? null : Number(r.avg_latency_ms),
      total_cost_usd:
        r.total_cost_usd == null ? null : Number(r.total_cost_usd),
...
function getPrimaryModel() {
  const row = db
    .prepare(
      "SELECT m.id, m.provider_id FROM model m JOIN provider p ON m.provider_id = p.id WHERE p.enabled = 1 ORDER BY m.is_primary DESC, m.id ASC LIMIT 1",
    )
    .get();
  return row
    ? { id: Number(row.id), provider_id: Number(row.provider_id) }
    : null;
}
function classifyIntent(text) {
  ...
  if (
    /\b(def |class |import |function|debug|bug|compile|error|git|bash|cmd|docker|sql|api|rest)\b/.test(
      t,
    ) ||
    /代码|编程|python|javascript|写一个|实现|算法|terminal/i.test(t)
  )
    return "coding";
  if (
    /数|算|方程|公式|证明|积分|导数|矩阵|定理|概率|统计|calculate|solve|math|equation/i.test(
      t,
    )
  )
    return "math";
  if (/翻译|english|chinese|translate|日语|英语|法语/i.test(t))
    return "translation";
...
function autoRoute(intent) {
  ...
  const scores = db
    .prepare(
      `SELECT ms.score, ms.model_id, m.model_name, m.provider_id, p.name as provider_name FROM model_score ms JOIN model m ON ms.model_id = m.id JOIN provider p ON m.provider_id = p.id WHERE ms.intent=? AND p.enabled=1 ORDER BY ms.score DESC LIMIT 1`,
    )
    .all(targetIntent);
  if (scores.length > 0) {
    const best = scores[0];
  ...
  const m2 = db
    .prepare(
      "SELECT m.id, m.model_name, m.provider_id, p.name as provider_name FROM model m JOIN provider p ON m.provider_id=p.id WHERE m.is_primary=1 AND p.enabled=1 LIMIT 1",
    )
    .get();
  if (m2)
    return {
  ...
  const m3 = db
    .prepare(
      "SELECT m.id, m.model_name, m.provider_id, p.name as provider_name FROM model m JOIN provider p ON m.provider_id=p.id WHERE p.enabled=1 ORDER BY m.id ASC LIMIT 1",
    )
    .get();
  if (m3)
    return {
...
function getRunnerUpModel(intent, excludeModelId) {
  ...
  const params = excludeModelId
    ? [targetIntent, excludeModelId]
    : [targetIntent];
  const scores = db
    .prepare(
      `
...
function getMemoriesScoped(workspace) {
  ...
  return db
    .prepare(
      "SELECT * FROM memory WHERE workspace IS NULL OR workspace = ? ORDER BY (workspace IS NULL) ASC, created_at DESC, id DESC",
    )
    .all(workspace);
}
function addMemory({ content, type, source_session_id, workspace }) {
  ...
  return addMemoryWithProvenance(
    content,
    type,
...
function addMemoryWithProvenance(
  content,
  type,
  sourceSessionId,
  origin,
  ...
  if (t === "relation" && relationMeta) {
    info = db
      .prepare(
        "INSERT INTO memory (content, type, relation_entity, relation_type, relation_target, source_session_id, confidence, origin, content_norm, workspace) VALUES (?, ?, ?, ?, ?, ?, 1.0, ?, ?, ?)",
      )
      .run(
        c,
        t,
        relationMeta.entity1 || null,
        relationMeta.relation || null,
        relationMeta.entity2 || null,
        sourceSessionId,
        origin || "user",
        memNormalize(c),
        workspace || null,
    ...
    info = db
      .prepare(
        "INSERT INTO memory (content, type, source_session_id, confidence, origin, content_norm, workspace) VALUES (?, ?, ?, 1.0, ?, ?, ?)",
      )
      .run(
        c,
        t,
  ...
  try {
    db.prepare(
      "INSERT INTO memories_fts (content, type, memory_id) VALUES (?, ?, ?)",
    ).run(c, t, Number(info.lastInsertRowid));
  } catch {}
...
function addMemoriesBatch(entries) {
  const insert = db.transaction((list) => {
    for (const e of list)
      addMemoryWithProvenance(e.content, e.type, e.sourceSessionId);
    return list.length;
  });
...
function updateMemory(id, { content }) {
  ...
  db.prepare(
    "UPDATE memory SET content = ?, content_norm = ? WHERE id = ?",
  ).run(content, memNormalize(content), id);
  try {
    db.prepare("UPDATE memories_fts SET content = ? WHERE memory_id = ?").run(
      String(content || ""),
      Number(id),
    );
  } catch {}
    ...
    const mergeTx = db.transaction(() => {
      for (const g of groups) {
        ...
        for (const row of toDelete) {
          ...
          ).run(g.keep_id, row.id);
          db.prepare("DELETE FROM memory WHERE id = ?").run(row.id);
          db.prepare("DELETE FROM memories_fts WHERE memory_id = ?").run(
            Number(row.id),
          );
          txRemoved++;
        }
...
function getMemoryConflicts() {
  try {
    const rows = db
      .prepare(
        "SELECT m.*, c.content as conflicting_content, c.type as conflicting_type FROM memory m JOIN memory c ON c.id = m.conflicts_with WHERE m.conflicts_with IS NOT NULL ORDER BY m.created_at DESC",
      )
      .all();
    return rows.map((r) => ({
      memoryId: r.id,
      content: r.content,
      type: r.type,
      conflictingId: r.conflicts_with,
      conflictingContent: r.conflicting_content,
      conflictingType: r.conflicting_type,
    }));
  } catch {
    return [];
  }
...
function resolveMemoryConflict(keepId, removeId) {
  try {
    db.prepare("DELETE FROM memory WHERE id = ?").run(removeId);
  } catch {}
  try {
    db.prepare("UPDATE memory SET conflicts_with = NULL WHERE id = ?").run(
      keepId,
    );
  } catch {}
}
...
function recordSkillResult(name, success) {
  try {
    db.prepare(
      "INSERT INTO skill_success (name, total_uses, successes, last_result) VALUES (?, 1, ?, ?) ON CONFLICT(name) DO UPDATE SET total_uses=total_uses+1, successes=successes+?, last_result=?, updated_at=CURRENT_TIMESTAMP",
...
function getSkillStats() {
  try {
    const rows = db
      .prepare(
        "SELECT name, total_uses, successes, last_result, updated_at FROM skill_success ORDER BY total_uses DESC",
      )
      .all();
    return rows.map((r) => ({
      name: r.name,
      ...
      successRate: r.total_uses > 0 ? (r.successes || 0) / r.total_uses : 0,
...
function initSkillSuccessTable() {
  try {
    db.exec(
      `CREATE TABLE IF NOT EXISTS skill_success (name TEXT PRIMARY KEY, total_uses INTEGER NOT NULL DEFAULT 0, successes INTEGER NOT NULL DEFAULT 0, last_result INTEGER DEFAULT 1, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)`,
    );
  } catch {}
}
...
function addAuditLog({ sessionId, turnId, payload }) {
  db.prepare(
    "INSERT INTO agent_execution_log (session_id, turn_id, payload) VALUES (?, ?, ?)",
  ).run(sessionId, turnId, JSON.stringify(payload));
}
...
function addAgentCheckpoint({
  ...
  const info = db
    .prepare(
      "INSERT INTO agent_checkpoint (session_id, message_id, tool_name, args, affected_paths, snapshot) VALUES (?, ?, ?, ?, ?, ?)",
    )
    .run(
      sessionId,
      messageId,
...
function getAgentCheckpoint(id) {
  ...
  try {
    row.affected_paths = JSON.parse(row.affected_paths || "[]");
  } catch {
    row.affected_paths = [];
  }
  try {
...
function markAgentCheckpointRolledBack(id) {
  db.prepare("UPDATE agent_checkpoint SET rolled_back_at = ? WHERE id = ?").run(
    localNow(),
    id,
  );
}
...
function getAuditLog(sessionId, limit = 50) {
  const rows = db
    .prepare(
      "SELECT * FROM agent_execution_log WHERE session_id = ? ORDER BY id DESC LIMIT ?",
    )
    .all(sessionId, limit);
  for (const row of rows)
    try {
...
function logUsage({
  ...
  db.prepare(
    "INSERT INTO usage_log (session_id, provider_id, provider_name, model_name, prompt_tokens, completion_tokens, total_tokens, cache_read_tokens, cache_creation_tokens, cost, latency_ms, status, source) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
  ).run(
    session_id,
    provider_id,
    provider_name,
...
function getUsageStats({ since = null, until = null } = {}) {
  ...
  const row = db
    .prepare(
      `SELECT COUNT(*) as requests, COALESCE(SUM(prompt_tokens),0) as prompt_tokens, COALESCE(SUM(completion_tokens),0) as completion_tokens, COALESCE(SUM(total_tokens),0) as total_tokens, COALESCE(SUM(cache_read_tokens),0) as cache_read_tokens, COALESCE(SUM(cache_creation_tokens),0) as cache_creation_tokens, COALESCE(SUM(cost),0) as cost, COALESCE(SUM(latency_ms),0) as latency_ms_sum, COUNT(latency_ms) as latency_count FROM usage_log ${w.where}`,
    )
    .get(...w.params);
  if (!row)
    return {
  ...
  return {
    ...
    latency_avg:
      row.latency_count > 0
        ? Number(row.latency_ms_sum) / Number(row.latency_count)
        : 0,
  };
}
function getUsageByProvider({ since = null, until = null } = {}) {
  ...
  return db
    .prepare(
      `SELECT provider_name, COUNT(*) as requests, COALESCE(SUM(total_tokens),0) as total_tokens, COALESCE(SUM(cost),0) as cost FROM usage_log ${w.where} GROUP BY provider_name ORDER BY cost DESC`,
    )
    .all(...w.params);
}
function getUsageByModel({ since = null, until = null } = {}) {
  ...
  return db
    .prepare(
      `SELECT model_name, COUNT(*) as requests, COALESCE(SUM(total_tokens),0) as total_tokens, COALESCE(SUM(cost),0) as cost FROM usage_log ${w.where} GROUP BY model_name ORDER BY cost DESC`,
    )
    .all(...w.params);
}
function getUsageDaily({ since = null, until = null } = {}) {
  ...
  return db
    .prepare(
      `SELECT date(created_at) as day, COUNT(*) as requests, COALESCE(SUM(total_tokens),0) as total_tokens, COALESCE(SUM(cost),0) as cost FROM usage_log ${w.where} GROUP BY day ORDER BY day ASC`,
    )
    .all(...w.params);
}
function getUsageLog({ limit = 200, since = null, until = null } = {}) {
  const w = buildRangeWhere(since, until);
  return db
    .prepare(`SELECT * FROM usage_log ${w.where} ORDER BY id DESC LIMIT ?`)
    .all(...w.params, limit);
}
...
function deleteAssistantAfterLastUser(sessionId) {
  const rows = db
    .prepare(
      "SELECT id, role FROM message WHERE session_id = ? ORDER BY id ASC",
    )
    .all(sessionId);
  let lastUserId = 0;
  for (const row of rows) {
  ...
  if (lastUserId > 0)
    db.prepare(
      "DELETE FROM message WHERE session_id = ? AND role = ? AND id > ?",
    ).run(sessionId, "assistant", lastUserId);
}
...
function deleteArenaAssistantMessages(sessionId) {
  db.prepare(
    "DELETE FROM message WHERE session_id = ? AND arena_model IS NOT NULL AND arena_model != ''",
  ).run(sessionId);
}

// ===== MCP server CRUD =====
function getMcpServers() {
  return db.prepare("SELECT * FROM mcp_server ORDER BY id").all();
}
function addMcpServer({ name, command, args = [], env = {}, enabled = 1 }) {
  const info = db
    .prepare(
      "INSERT INTO mcp_server (name, command, args, env, enabled) VALUES (?, ?, ?, ?, ?)",
    )
    .run(
      name,
      command,
...
function updateMcpServer(id, data) {
  ...
  const serialized = keys.map((k) =>
    k === "args" || k === "env" ? JSON.stringify(data[k]) : data[k],
  );
  db.prepare(
    `UPDATE mcp_server SET ${keys.map((k) => `${k} = ?`).join(", ")} WHERE id = ?`,
  ).run(...serialized, id);
}
  ...
  try {
    ...
    if (ftsAvailable) {
      ...
      const rows = sessionId
        ? db.prepare(sql).all(ftsQuery, sessionId)
    ...
    const sql = sessionId
      ? "SELECT * FROM message WHERE content LIKE ? ESCAPE '!' AND session_id = ? ORDER BY id DESC LIMIT 50"
      : "SELECT * FROM message WHERE content LIKE ? ESCAPE '!' ORDER BY id DESC LIMIT 50";
    return sessionId
      ? db.prepare(sql).all(like, sessionId)
      : db.prepare(sql).all(like);
  } catch {
    return [];
  }
}
function searchMemories(rawQuery) {
  try {
    const ftsAvailable =
      db
        .prepare("SELECT name FROM sqlite_master WHERE name = 'memories_fts'")
        .all().length > 0;
    if (ftsAvailable) {
      ...
      const rows = db
        .prepare(
          "SELECT memory_id FROM memories_fts WHERE memories_fts MATCH ? ORDER BY rowid DESC LIMIT 30",
        )
        .all(query);
      return rows
        .map(
          (r) =>
            db.prepare("SELECT * FROM memory WHERE id = ?").get(r.memory_id) ||
            null,
        )
        .filter(Boolean);
    }
    ...
    return db
      .prepare(
        "SELECT * FROM memory WHERE content LIKE ? ESCAPE '!' ORDER BY id DESC LIMIT 30",
      )
      .all(like);
  } catch {
    return [];
...
function getRepoIndexCache(workspace) {
  ...
  try {
    return (
      db
        .prepare(
          "SELECT mtime_x, graph_json FROM repo_index_cache WHERE workspace=?",
        )
        .get(workspace) || null
    );
  } catch {
...
function setRepoIndexCache(workspace, mtimeX, graphJson) {
  ...
  try {
    db.prepare(
      "INSERT INTO repo_index_cache (workspace, mtime_x, graph_json) VALUES (?, ?, ?) ON CONFLICT(workspace) DO UPDATE SET mtime_x=excluded.mtime_x, graph_json=excluded.graph_json",
...
function deleteRepoIndexCache(workspace) {
  if (!db || !workspace) return;
  try {
    db.prepare("DELETE FROM repo_index_cache WHERE workspace=?").run(workspace);
  } catch {}
}
...
function getSessionPlan(sessionId) {
  const row = db
    .prepare("SELECT plan_json FROM session_plan WHERE session_id = ?")
    .get(sessionId);
  if (!row) return null;
  try {
...
function saveSessionPlan(sessionId, planObj) {
  db.prepare(
    'INSERT INTO session_plan (session_id, plan_json, updated_at) VALUES (?, ?, datetime("now")) ON CONFLICT(session_id) DO UPDATE SET plan_json = excluded.plan_json, updated_at = excluded.updated_at',
  ).run(sessionId, JSON.stringify(planObj));
}
...
function getSkillUsage() {
  try {
    return db
      .prepare("SELECT * FROM skill_usage ORDER BY use_count DESC")
      .all();
  } catch {
    return [];
  }
...
function updateSkillState(name, state) {
  try {
    if (state === "archived")
      db.prepare(
        "UPDATE skill_usage SET state = ?, archived_at = datetime('now') WHERE name = ?",
      ).run(state, name);
    else
      db.prepare("UPDATE skill_usage SET state = ? WHERE name = ?").run(
        state,
        name,
      );
  } catch {}
...
function pinSkill(name, pinned) {
  try {
    db.prepare("UPDATE skill_usage SET pinned = ? WHERE name = ?").run(
      pinned ? 1 : 0,
      name,
    );
  } catch {}
...
function applySkillTransitions() {
  try {
    db.prepare(
      "UPDATE skill_usage SET state = 'stale' WHERE state = 'active' AND pinned = 0 AND last_used_at IS NOT NULL AND julianday('now') - julianday(last_used_at) > 30",
    ).run();
    db.prepare(
      "UPDATE skill_usage SET state = 'archived', archived_at = datetime('now') WHERE state IN ('active','stale') AND pinned = 0 AND last_used_at IS NOT NULL AND julianday('now') - julianday(last_used_at) > 90",
    ).run();
    ...
    db.prepare(
      "UPDATE skill_usage SET state = 'stale' WHERE state = 'active' AND pinned = 0 AND name IN (SELECT name FROM skill_success WHERE total_uses >= 5 AND (successes * 1.0 / total_uses) < 0.5)",
    ).run();
  } catch {}
}
...
function getCompactionState(sessionId) {
  return db
    .prepare(
      "SELECT split_index, summary FROM compaction_state WHERE session_id = ?",
    )
    .get(sessionId);
}
...
function saveCompactionState(sessionId, splitIndex, summary) {
  db.prepare(
    `INSERT INTO compaction_state (session_id, split_index, summary, updated_at)
              VALUES (?, ?, ?, datetime('now'))
...
function deleteCompactionState(sessionId) {
  db.prepare("DELETE FROM compaction_state WHERE session_id = ?").run(
    sessionId,
  );
}
...
module.exports = {
  ...
  isMaskedOrEmptyKey,
  sanitizeSkillName,
  getModels, getAllModels, getModel, addModel, updateModel, deleteModel, syncModels, getFallbackChain,
  getPersonas,
  getPersona,
  addPersona,
  updatePersona,
  ...
  listArenaBenchmarks,
  saveArenaBenchmark,
  deleteArenaBenchmark,
  updateArenaBenchmarkResults,
  saveDatabase,
  flushDatabase,
  getPrimaryModel,
  getSessionConfig,
  ...
  addNormalMessage: function ({ session_id, role, content, model_used }) {
    db.prepare(
      "INSERT INTO message (session_id, role, content, model_used, created_at) VALUES (?, ?, ?, ?, datetime('now'))",
    ).run(session_id, role, content, model_used || null);
  },
  getMcpServers,
  addMcpServer,
  updateMcpServer,
  ...
  getRepoIndexCache,
  setRepoIndexCache,
  deleteRepoIndexCache,
  getSkillUsage,
  updateSkillState,
  pinSkill,
  applySkillTransitions,
  listCredentials: function (pid) {
    return require("./llm/credentialPool").listCredentials(pid);
  },
  addCredential: function (pid, key, label) {
    return require("./llm/credentialPool").addCredential(pid, key, label);
  },
  removeCredential: function (cid) {
    return require("./llm/credentialPool").removeCredential(cid);
  },
  prepare: (...args) => (db ? db.prepare(...args) : null),
  run: (...args) => {
    if (db) return db.prepare(args[0]).run(...args.slice(1));
  },
