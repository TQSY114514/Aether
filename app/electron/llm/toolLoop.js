...
CODE EDITING RULES:
- Prefer \`apply_patch\` (unified diff or <<<<<<< SEARCH ... ======= ... >>>>>>> REPLACE blocks) or \`edit_file\` for targeted modifications to existing files instead of overwriting with \`write_file\`.
- Only use \`write_file\` when creating new files or when completely replacing the file content is genuinely necessary.
- In \`apply_patch\`, provide exact search context matching existing code.
