...
const { getWorkspaceRoot, isAuthorizedWorkspace } = require('../tools/sandbox')
...
function registerGitHandlers(ipcMain, db) {
  ...
  ipcMain.handle('git:undo', (_e, cwdOrOpts) => {
    ...
    if (!isAuthorizedWorkspace(db, root)) return { success: false, error: 'unauthorized workspace path' }
