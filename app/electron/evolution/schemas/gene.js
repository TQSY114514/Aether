...
const GENE_DEFAULTS = {
  type: 'Gene',
  id: null,
  category: 'innovate',  // repair | optimize | innovate | explore
  signals_match: [],
  strategy: [],
  validation: [],
  ...
  learning_history: [],
  anti_patterns: [],
  routing_hint: null,  // { tier: 'cheap|mid|expensive', reasoning_level: 'off|low|medium|high' }
  tool_policy: null,   // { allow_only: [], deny: [], severity: 'warn|block' }
};
...
function createGene(partial) {
  ...
  if (typeof g.constraints.max_files !== 'number' || g.constraints.max_files < 1) {
  ...
  // Normalize optional routing_hint / tool_policy
...
function validateGene(g) {
  ...
  if (!Array.isArray(g.validation))         throw new Error('Gene.validation must be an array');
  if (!g.constraints || typeof g.constraints !== 'object')
                                            throw new Error('Gene.constraints must be an object');
  if (typeof g.constraints.max_files !== 'number')
                                            throw new Error('Gene.constraints.max_files must be a number');
  if (!Array.isArray(g.constraints.forbidden_paths))
                                            throw new Error('Gene.constraints.forbidden_paths must be an array');
  return true;
}
