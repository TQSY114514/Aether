...
const encryptedApiKey = process.env.AETHER_TEST_ENCRYPTED_API_KEY
  || Buffer.from('test-only-encrypted-key').toString('base64')
...
describe('runSession', () => {
  ...
  it('rejects encrypted safeStorage API keys with a clear error (headless)', async () => {
    const encResolve = () => ({
      provider: { id: 1, name: 'my-provider', api_url: 'http://x', api_key: encryptedApiKey, api_format: 'openai' },
  ...
  it('falls back to auth.json when stored key is encrypted (persisted via /apikey)', async () => {
    ...
    try {
      let got
      const encResolve = () => ({
        provider: { id: 1, name: '新疆', api_url: 'http://x', api_key: encryptedApiKey, api_format: 'openai' },
        model: { id: 1, model_name: 'm' },
        db: null,
      })
      await runSession({
